...
export default function CountryPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["country-manga", country, page],
    queryFn: () => searchManga(countryName, page, ITEMS_PER_PAGE),
    staleTime: 2 * 60 * 1000,
  });
  ...
  return (
    <div className="container py-8">
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold">Truyện {countryName}</h1>
          <p className="text-sm text-muted-foreground">
            Tổng hợp truyện đến từ {countryName}
          </p>
        </div>
      </div>
